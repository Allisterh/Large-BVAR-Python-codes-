...
def bvarIrfs(beta, sigma, nshock, hmax):
    ...
    :param nshock: Position of the shock
    :param hmax: Maximum horizon for the impulse response
    :return: irf (Impulse response function)
    """
    k, n = beta.shape
    lags = (k - 1) // n
    ...
    for tau in range(1, hmax + 1):
        xT = Y[in_ + tau - 1:in_ + tau - lags - 1:-1, :].T.reshape(k - 1, 1).T
