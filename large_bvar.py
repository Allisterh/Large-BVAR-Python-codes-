...
def FIS(Y, Z, R, T, Q, S):
    """
    Fixed Interval Smoother (FIS) based on Harvey, 1989, p. 154.

    This function returns the smoothed state vector AmT and its covariance matrix PmT.
    Use this in conjunction with function SKF (Kalman filter).

    Args:
        Y (numpy.ndarray): Data array of shape (nobs, n)
        Z (numpy.ndarray): System matrix Z, not used in this function but kept for compatibility
        R (numpy.ndarray): System matrix R, not used in this function but kept for compatibility
        T (numpy.ndarray): System matrix T
        Q (numpy.ndarray): System matrix Q, not used in this function but kept for compatibility
        S (dict): Dictionary containing estimates from Kalman filter SKF
            S['Am']: Estimates \(a_{t|t-1}\) of shape (nobs, m)
            S['Pm']: \(P_{t|t-1} = \text{Cov}(a_{t|t-1})\) of shape (nobs, m, m)
            S['AmU']: Estimates \(a_{t|t}\) of shape (nobs, m)
            S['PmU']: \(P_{t|t} = \text{Cov}(a_{t|t})\) of shape (nobs, m, m)

    Returns:
        dict: Dictionary containing smoothed estimates added to S
            S['AmT']: Estimates \(a_{t|T}\) of shape (nobs, m)
            S['PmT']: \(P_{t|T} = \text{Cov}(a_{t|T})\) of shape (nobs, m, m)
            S['PmT_1']: Cov(\(a_{t}a_{t-1}|T\)) of shape (nobs, m, m)
    """
    ...
    # Initial values
    S['AmT'][:, nobs - 1] = np.squeeze(S['AmU'][:, nobs - 1])  # nobs - 1 as python has zero-based indexing
    S['PmT'][:, :, nobs - 1] = np.squeeze(S['PmU'][:, :, nobs - 1])

    # Compute initial J_2
    J_2 = np.dot(np.dot(np.squeeze(S['PmU'][:, :, nobs - 1]), T.T), pinv(np.squeeze(S['Pm'][:, :, nobs - 1])))

    for t in range(nobs, 0, -1):
        # Extract necessary matrices and vectors
        PmU = np.squeeze(S['PmU'][:, :, t - 1])
        Pm1 = np.squeeze(S['Pm'][:, :, t - 1])
        P_T = np.squeeze(S['PmT'][:, :, t])

        # Update J_1
        J_1 = J_2

        # Update smoothed state estimate and its covariance
        S['AmT'][:, t - 1] = S['AmU'][:, t - 1] + np.dot(J_1, (S['AmT'][:, t] - np.dot(T, S['AmU'][:, t - 1])))
        S['PmT'][:, :, t - 1] = PmU + np.dot(np.dot(J_1, (P_T - Pm1)), J_1.T)

        if t > 1:
            # Update J_2 for the next iteration
            # J_2 = np.dot(np.dot(np.squeeze(S['PmU'][:, :, t - 2]), T.T), pinv(np.squeeze(S['Pm'][:, :, t - 2])))
            J_2 = np.dot(np.dot(np.squeeze(S['PmU'][:, :, nobs - 1]), T.T), pinv(np.squeeze(S['Pm'][:, :, nobs - 1])))

            # Update cross-covariance
            S['PmT_1'][:, :, t - 2] = np.dot(PmU, J_2.T) + np.dot(np.dot(J_1, (P_T - np.dot(T, PmU))), J_2.T)
