...
def derivest(fun, x0, varargin):
    ...
    if (par['DerivativeOrder'] % 2 == 0) or (not par['Style'].lower().startswith('central')):
        if par['Vectorized'].lower() == 'yes':
            ...
            for j, val in np.ndenumerate(x0):
                f_x0[j] = fun(val)
    else:
        f_x0 = None
...
def hessian(fun: Callable, x0: np.array, epsilon: float = 1.e-5) -> np.array:
    """
    Compute the Hessian matrix for a given function at a given point.

    This function estimates the Hessian matrix using the finite difference method.
    It approximates the gradient vectors around the point of interest and uses them
    to build the Hessian.

    Args:
        fun (Callable): The function for which the Hessian is to be calculated. Must be a function of x0.
        x0 (np.array): The point at which the Hessian is to be calculated.
        epsilon (float, optional): The small change applied to each coordinate to compute the partial derivatives.
         Default is 1.e-5.

    Returns:
        np.array: The Hessian matrix of `fun` at `x0`.

    Example:
        >>> def func(x):
        ...     return x[0]**2 + x[1]**2
        >>> hessian(func, np.array([1, 1]))
        array([[2., 0.],
               [0., 2.]])
    """
    ...
    for i in range(n):
        # Create points x1 and x2 such that x1 = x0 - epsilon along the i-th coordinate
        # and x2 = x0 + epsilon along the i-th coordinate
        x1 = np.array(x0, dtype=float)
        x2 = np.array(x0, dtype=float)
        x1[i] -= epsilon
        x2[i] += epsilon

        # Approximate the gradients at x1 and x2
        gradient1 = approx_fprime(x1, fun, epsilon=[epsilon] * n)
        gradient2 = approx_fprime(x2, fun, epsilon=[epsilon] * n)

        # Compute the i-th column (or row since Hessian is symmetric) of the Hessian matrix
        hessian_matrix[i] = (gradient2 - gradient1) / (2 * epsilon)
...
def hessdiag(fun, x0):
    ...
    for ind in range(nx):
        ...
        fun_handle = lambda xi: fun(np.array(swapelement(x0.flatten().tolist(), ind, xi)).reshape(sx))
...
def logMLVAR_formcmc_covid(par, y, x, lags, T, n, b, MIN, MAX, SS, Vc, pos, mn, sur, noc, y0, draw,
    ...
    if np.any(np.concatenate([np.array([lambda_]), eta.ravel(), np.array([theta, miu, alpha])]) <
        ...
        if sur == 1 or noc == 1:
            ...
            term3 = (betahatd - b).T @ np.diag(1. / omega_1D) @ (betahatd - b)
